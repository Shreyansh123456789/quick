...
const getGroupsList = (groupBy, tickets) => {
    if (groupBy === "status") return ["Backlog", "Todo", "In progress", "Done", "Canceled"];
    if (groupBy === "priority") return [0, 1, 2, 3, 4];

    const groupsSet = new Set();
    tickets.forEach(ticket => groupsSet.add(ticket[groupBy === "user" ? "userId" : groupBy]));
...
const mapGroupLabelsAndIcons = (groupBy, value, users) => {
    if (groupBy === "priority") {
        ...
        return {
            label: user.name,
            icon: <Avatar size="20" round={true} textSizeRatio={1.75} color={getRandomColor(user.name)} name={user.name} />,
        };
    } else {
        return {
            label: value,
...
const sortTickets = (tickets, groupBy, sortBy) => {
    tickets.sort((a, b) => {
        ...
        if (groupBy === "priority" || (sortBy === "priority" && propertyA === propertyB)) {
...
const groupAndAssignTickets = (tickets, groupBy, groups, users) => {
    tickets.forEach((ticket) => {
        ...
        if (groupBy === "priority") groups.find((group) => group.value === ticket.priority)?.tickets.push(ticket);
        else if (groupBy === "user") groups.find((group) => group.value === ticket.userId)?.tickets.push(ticket);
        else if (groupBy === "status") groups.find((group) => group.value === ticket.status)?.tickets.push(ticket);
    })
}
...
const GroupTable = ({ tickets, groupBy, users, sortBy }) => {
    ...
    return (
        <div style={groupTableStyle}>
            {
                getGroups(groupBy, tickets, users, sortBy).map((group) => <GroupColumn group={group} />)
            }
        </div>)
}
